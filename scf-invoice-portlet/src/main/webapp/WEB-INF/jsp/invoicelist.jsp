...
<div class="container-fluid" style="padding: 0px;">
	...
	<div class="row-fluid">
		<div class="span12">
			<div class="span12">
				<h6>Conveniently manage your invoices.Please select one or more
					invoices to create trade.Alternatively you can upload invoices to
					create new trade</h6>
			</div>
	...
	<form name="invoicelist" method="post" action="${invoiceDocumentsURL}"
		id="invoicelist">
		<input type="hidden" name="invoices" id="invoices"> <input
			type="hidden" name="companyId" id="companyId"> <input
			type="hidden" name="currentPage" id="currentPage"
			value="${paginationModel.currentPage}" /> <input type="hidden"
			name="currPageSize" id="currPageSize"
			value="${paginationModel.pageSize}" /> <input type="hidden"
		...
		<div class="row-fluid">
			<div class="span6">
				<input type="button" value="Search" id="invoiceReport"
		...
		<div class="row-fluid">
			...
			<div class="span6">
				<c:if
					...
					<input type="button" value="Create Invoice" class="btn btn-primary"
						id="createInvoice" data-url="${createInvoiceURL}" class="span6" />
				</c:if>
				<c:choose>
					<c:when test="${userType eq 'Seller Admin' }">
						<input type="button" value="Request Finance"
							class="btn btn-primary" id="requestFinance"
							data-url="${requestFinanceURL}" class="span6" />
					</c:when>
					<c:otherwise>
						<input type="button" value="Create Trade" class="btn btn-primary"
							id="createTrade" data-url="${createTradeURL}" class="span6" />
					</c:otherwise>
				</c:choose>
			</div>
			<div class="span6">

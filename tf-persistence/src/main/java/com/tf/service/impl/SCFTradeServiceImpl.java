...
public class SCFTradeServiceImpl implements SCFTradeService {
	...
	public void updateTradeLifeCycle(SCFTrade scfTrade) {
		...
		if (TradeStatus.ALLOTMENT_PAID.getValue().equalsIgnoreCase(
				scfTrade.getStatus())) {
			List<InvestorTransaction> transcations = investorTransactionDAO
					.getInvestorTransactionByTrade(scfTrade.getId());
			...
			for (InvestorTransaction investorTransaction : transcations) {
				...
				invTransaction.setInvestorID(investorTransaction
						.getInvestorID());

				// updating investor Cash Position
				Investor inv = investorDAO.findByInvestorId(investorTransaction
						.getInvestorID());
				inv.setCashPosition(inv.getCashPosition().subtract(
						investorTransaction.getAmount()));
				inv.setUpdateDate(date);
				investorDAO.updateInvestor(inv);

				invTransaction.setAmount(investorTransaction.getAmount());
				invTransaction.setTranscationType(TranscationStatus.INVESTED
						.getValue());
		...
		} else if (TradeStatus.SUPPLIER_PAID.getValue().equalsIgnoreCase(
				scfTrade.getStatus())) {
			...
			whtTransaction.setTranscationType(TradeStatus.SUPPLIER_PAID
					.getValue());
			whiteHallTransactionDAO.saveEntity(whtTransaction);
			tradeUpdate = Boolean.TRUE;

		} else if (TradeStatus.SCF_REPAYMENT.getValue().equalsIgnoreCase(
				scfTrade.getStatus())) {
			...
			whtTransaction.setTranscationDate(new Date());
			whtTransaction.setTradeID(scfTrade.getId());
			whtTransaction.setTranscationType(TradeStatus.SCF_REPAYMENT
					.getValue());
			whiteHallTransactionDAO.saveEntity(whtTransaction);
			tradeUpdate = Boolean.TRUE;

		} else if (TradeStatus.INVESTOR_PAID.getValue().equalsIgnoreCase(
				scfTrade.getStatus())) {
			updateAllotmentInvPaid(scfTrade,
					TradeStatus.INVESTOR_PAID.getValue());
			...
			whtTransaction.setTranscationDate(new Date());
			whtTransaction.setTradeID(scfTrade.getId());
			whtTransaction.setTranscationType(TradeStatus.INVESTOR_PAID
					.getValue());
			whiteHallTransactionDAO.saveEntity(whtTransaction);

			// Adding Whitehall investor profit paid Transaction
			WhiteHallTransaction whtTransactionInvProfit = new WhiteHallTransaction();
			whtTransactionInvProfit.setAmount(allotmentDAO
					.getTotalInvestorProfitForTrade(scfTrade.getId()));
			whtTransactionInvProfit
					.setReference("SCF Company Paid to Whitehall Admin");
			whtTransactionInvProfit.setTranscationDate(new Date());
			whtTransactionInvProfit.setTradeID(scfTrade.getId());
			whtTransactionInvProfit
					.setTranscationType(TradeStatus.INVESTOR_PAID.getValue());
			whiteHallTransactionDAO.saveEntity(whtTransactionInvProfit);
			tradeUpdate = Boolean.TRUE;
		} else if (TradeStatus.SETTLED.getValue().equalsIgnoreCase(
				scfTrade.getStatus())) {
	...
	private void updateAllotments(SCFTrade scfTrade, String status) {
		List<Allotment> allotments = allotmentDAO.getALlotmentsbyTrade(scfTrade
				.getId());
	...
	private void updateAllotmentInvPaid(SCFTrade scfTrade, String status) {
		...
		for (Allotment allotment : allotments) {
			...
			// profit
			BigDecimal netAmmount = allotment.getAllotmentAmount().add(
					allotment.getInvestorNetProfit());
			...
			invTransactionRepaid.setTranscationType(TranscationStatus.REPAID
					.getValue());
			invTransactionRepaid.setTranscationDate(date);
			invTransactionRepaid.setTradeID(scfTrade.getId());
			invTransactionRepaid.setReference(TranscationStatus.REPAID
					.getValue());
			...
			invTransactionProfit.setTranscationType(TranscationStatus.PROFIT
					.getValue());
			invTransactionProfit.setTranscationDate(date);
			invTransactionProfit.setTradeID(scfTrade.getId());
			invTransactionProfit.setReference(TranscationStatus.PROFIT
					.getValue());
			...
			invTransactionFee
					.setTranscationType(TranscationStatus.WHITEHALL_FEE
							.getValue());
			invTransactionFee.setTranscationDate(date);
			invTransactionFee.setTradeID(scfTrade.getId());
			invTransactionFee.setReference(TranscationStatus.WHITEHALL_FEE
					.getValue());
	...
	public List<SCFTrade> getScfTrades(Long companyID, int startIndex,
			int pageSize) {
	...
	public List<SCFTrade> getScfTradeList(String RegNum, int startIndex,
			int pageSize) {
		List<SCFTrade> scfTrades = scfTradeDAO.getScfTradeList(RegNum,
				startIndex, pageSize);
		return scfTrades;
	}
	...
	public List<SCFTrade> getScfTradeListWithSearch(String searchtxt,
		...
		List<SCFTrade> scfTrades = scfTradeDAO.getScfTradeListWithSearch(
				searchtxt, RegNum, startIndex, pageSize);
		return scfTrades;
	}
	...
	public List<SCFTrade> getTradeHistoryByComapnyId(long companyId,
			int startIndex, int pageSize) {
		return scfTradeDAO.getTradeHistoryByComapnyId(companyId, startIndex,
				pageSize);
	}
	...
	public List<SCFTrade> getScfTradeByScfCompany(String scfCompany,
			Date frmDate, Date toDate, int startIndex, int pageSize) {
		return scfTradeDAO.getScfTradeByScfCompany(scfCompany, frmDate, toDate,
				startIndex, pageSize);
	}

	public List<SCFTrade> getScfTradeSellerCompany(String scfCompany,
			Date frmDate, Date toDate, long compID, int startIndex, int pageSize) {
		return scfTradeDAO.getScfTradeSellerCompany(scfCompany, frmDate,
				toDate, compID, startIndex, pageSize);
	}

	public Long getScfTradeSellerCompanyCount(String scfCompany, Date frmDate,
			Date toDate, long compID) {
		return scfTradeDAO.getScfTradeSellerCompanyCount(scfCompany, frmDate,
				toDate, compID);
	}

	public Long getScfTradeByScfCompanyCount(String scfCompany, Date frmDate,
			Date toDate) {
		return scfTradeDAO.getScfTradeByScfCompanyCount(scfCompany, frmDate,
				toDate);
	}
	...
	public List<SCFTrade> getAdminTradeListWithSearch(String searchtxt,
			Date fromDate, Date toDate, String value, int startIndex,
			int pageSize) {
		return scfTradeDAO.getAdminTradeListWithSearch(searchtxt, fromDate,
				toDate, value, startIndex, pageSize);
	}
}

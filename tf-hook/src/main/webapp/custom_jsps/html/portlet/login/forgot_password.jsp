	...
	<!-- <aui:fieldset> -->
		<c:choose>
			<c:when test="<%= user2 == null %>">
				...
				<div class="footer">
				<aui:button-row>
					<aui:button type="submit"  cssClass="btn-block btnBgBuSm"   value='<%= PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "next" : "send-new-password" %>' />
				</aui:button-row>
				</div>
			...
			<c:when test="<%= (user2 != null) && Validator.isNotNull(user2.getEmailAddress()) %>">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<aui:button-row>
							<aui:button cssClass="btn-block btnBgBuSm" type="submit" value='<%= company.isSendPasswordResetLink() ? "send-password-reset-link" : "send-new-password" %>' />
						</aui:button-row>
					</c:otherwise>
				</c:choose>
			</c:when>
